...
static int globalvar_init(void)
{
    ...
    */
    ...
    //仅对control寄存器进行ioremap，data寄存器由用户通过mmap直接映射
...
static int my_map(struct file *filp, struct vm_area_struct *vma)
 {    
     ...
     //若用户传递offset为0即vm_pgoff =0，那么驱动需自己知道硬件地址，
     //此时需要做一下物理地址到页帧率的转化 即PA_ADDR >> PAGE_SHIFT
     unsigned long offset = vma->vm_pgoff;
     //unsigned long offset = GPJ0DATA_PA >> PAGE_SHIFT;
     unsigned long start = (unsigned long)vma->vm_start;
     //unsigned long end =  (unsigned long)vma->vm_end;
     unsigned long size = (unsigned long)(vma->vm_end - vma->vm_start);
     printk(KERN_WARNING"mmap being called! addr =%x\n", vma->vm_pgoff);
     //得到物理地址
     //page = virt_to_phys(buffer);    
     //将用户空间的一个vma虚拟内存区映射到以page开始的一段连续物理页面上
     ret = remap_pfn_range(vma,start,offset,size,PAGE_SHARED);//第三个参数是页帧号，由物理地址右移PAGE_SHIFT得到
     if (ret)
     {   
         printk(KERN_WARNING"remap_pfn_range error ret = %d\n", ret);//打印出传入的参数
