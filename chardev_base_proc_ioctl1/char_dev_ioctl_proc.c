...
static int globalvar_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case MEM_READ: //假如这里传入的是重启的命令
			copy_from_user(&kernelparam,(param *)arg,sizeof(param));
			kernelparam.num = 2;
                        kernelparam.id  = 2;
                        copy_to_user((param *)arg,&kernelparam,sizeof(param));
                        copy_to_user((unsigned int *)(kernelparam.useraddr),a,5);
                        copy_to_user(kernelparam.puseraddr,a,5);
